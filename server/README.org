...
So, defining a function to retrive an item is as follow:

#+BEGIN_SRC lisp :tangle back-end.lisp
(defun get-cards (&key (page 0) (per-page 20))
  (with-connection *config*
    (query-dao 'card-result
     "SELECT c.id, 
	CURRENT_DATE - DATE(c.creationDate) AS daysSinceCreated,
	a.sla, c.patientId, c.healthInsuranceId,
	c.visitId, c.billId,
	bt.name AS billType, b.totalAmount,
	COUNT(pend.id) AS numberOfPendencies,
	COUNT(pend.open) AS numberOfOpenPendencies,
	COUNT(d.id) AS numberOfnumberOfDocuments,
	COUNT(d.notReceived) AS numberOfNotReceivedDocuments,
	COUNT(ci.id) AS numberOfChecklistItem,
	COUNT(ci.done) AS numberOfDoneChecklistItem
      FROM cards c
